...
/*   Updated: 2019/02/06 10:36:04 by gdrai            ###   ########.fr       */
/*                                                                            */
/* ************************************************************************** */

#include "wolf3d.h"

void			wolf_init_img_c(t_infoswind *window, t_img *img, void *mlx_ptr)
{
	if (!(img->mossy = mlx_xpm_file_to_image(mlx_ptr, "./ressources/mossy.XPM",
		&window->size_texture, &window->size_texture)))
		wolf_err(2);
	...
	if (!(img->bluestone = mlx_xpm_file_to_image(mlx_ptr,
		"./ressources/bluestone.XPM", &window->size_texture,
			&window->size_texture)))
		wolf_err(2);
	img->data_texture[5] = (char *)mlx_get_data_addr(img->bluestone,
		&img->bperpix, &img->size_line_texture, &img->endian);
	if (!(img->eagle = mlx_xpm_file_to_image(mlx_ptr, "./ressources/eagle.XPM",
		&window->size_texture, &window->size_texture)))
		wolf_err(2);
...
void			wolf_init_img_b(t_infoswind *window, t_img *img, void *mlx_ptr)
{
	if (!(img->colorstone = mlx_xpm_file_to_image(mlx_ptr,
		"./ressources/colorstone.XPM", &window->size_texture,
			&window->size_texture)))
		wolf_err(2);
...
void			wolf_init_img(t_infoswind *window, t_img *img, void *mlx_ptr)
{
	...
	if (!(img->mini_map = mlx_new_image(mlx_ptr, window->size_mini_map,
		window->size_mini_map)))
		wolf_err(2);
	...
	img->data_xpm = (char *)mlx_get_data_addr(img->xpm_img,
		&img->bperpix, &img->size_line, &img->endian);
	wolf_init_img_b(window, img, mlx_ptr);
}
